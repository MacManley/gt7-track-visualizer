...
def createline(filename, xOffset, yOffset, zOffset):
    ...
    return go.Scatter3d(x=-df['posX'] + xOffset, y=df['posZ'] + yOffset, z=df['posY'] + zOffset, name=filename, marker=dict(size=1), line=dict(width=20, color=df['speedKMH'], colorscale='Temps_r', cmin=-min(df['speedKMH']), cmax=max(df['speedKMH'])))
